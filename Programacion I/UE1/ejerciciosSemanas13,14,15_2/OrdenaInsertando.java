...
//**** ORDENACIÓN DE UN ARRAY, UTILIZANDO OTRO AUXILIAR EN EL QUE SE VAN INSERTANDO UNO A UNO EN ORDEN LOS ELEMENTOS DEL PRIMERO.
...
public class OrdenaInsertando {
  ...
  static int[] ordenacionInsercion(int[] arr){
    //BORRAD LA SENTENCIA DE DEBAJO Y COMPLETAD EL CODIGO
    //DEBEIS UTILIZAR EL METODO inserOrden QUE TENEIS CODIFICADO MAS ARRIBA
    return result;
  }
  ...
  private static void do_check(int[] arr, int[] solucion){
    try{
      ...
      if (!Arrays.equals(solAlumno,solucion)){
        System.out.println("Error: el código del alumno produce un resultado incorrecto.");
        System.out.println("El resultado esperado para el argumento: " + 
                           Arrays.toString(arr));    
        System.out.println("debería ser " + Arrays.toString(solucion));
        System.out.println("pero el resultado obtenido es " + Arrays.toString(solAlumno));
        throw new Error();
      }
    } catch (Exception e){
      System.out.println("**** El código del alumno ha lanzado una excepción: " + e.toString());
      throw new Error();
    }
